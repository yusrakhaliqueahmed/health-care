...
export const ClinicalOutputCard: React.FC<ClinicalOutputCardProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Honest AI-Only vs Doctor-Reviewed Transparency Banner */}
      <div className="px-4 sm:px-6 py-2.5 bg-slate-50 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between gap-3 text-xs">
        <div className="flex items-center gap-2">
          <AlertCircle className="w-4 h-4 text-amber-600 dark:text-amber-400 shrink-0" />
          <span className="text-slate-600 dark:text-slate-300 font-medium text-[11px] sm:text-xs">
            {isUrdu ? (
              <>
                <strong className="text-slate-900 dark:text-white font-bold">صرف اے آئی تجزیہ:</strong> یہ معلومات کمپیوٹر الگورتھم سے تیار کی گئی ہیں اور ابھی تک کسی مستند ڈاکٹر نے تصدیق نہیں کی۔
              </>
            ) : isRoman ? (
              <>
                <strong className="text-slate-900 dark:text-white font-bold">Sirf AI Tajziya:</strong> Yeh nateeja computer algorithm ne tayyar kiya hai aur abhi tak kisi doctor ne review nahi kiya.
              </>
            ) : (
              <>
                <strong className="text-slate-900 dark:text-white font-bold">AI-Generated Only:</strong> This assessment is generated by AI and has not yet been reviewed by a licensed doctor.
              </>
            )}
          </span>
        </div>
        <span className="shrink-0 px-2 py-0.5 rounded-md bg-amber-100 dark:bg-amber-950 text-amber-800 dark:text-amber-200 text-[10px] font-bold">
          {isUrdu ? 'ڈاکٹر معائنہ باقی ہے' : 'Not Doctor Reviewed'}
        </span>
      </div>
