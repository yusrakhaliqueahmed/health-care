        ...
        <div className="space-y-4">
          ...
          <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-900 dark:text-amber-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-xs">
            <div className="flex items-start gap-2.5">
              ...
              <div>
                <span className="font-bold block text-xs mb-0.5 text-amber-700 dark:text-amber-300">
                  {t.doctorNoticeTitle}
                </span>
                <p className="text-xs leading-relaxed opacity-90">
                  {t.doctorDemoNoticeBanner}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2 shrink-0 self-end sm:self-center">
              <button
                type="button"
                onClick={() => voiceManager.speak(t.doctorDemoNoticeBanner, currentLanguage)}
                className="px-3 py-1.5 rounded-xl bg-amber-500/20 hover:bg-amber-500/30 text-amber-700 dark:text-amber-300 text-xs font-bold flex items-center gap-1.5 transition-all"
              ...
              {onOpenDoctorOnboarding && (
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl bg-teal-600 hover:bg-teal-500 text-white text-xs font-bold transition-all shadow-xs"
                >
                  {t.joinAsDoctor}
                </button>
              )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {filteredDoctors.map((doc) => {
              const isRealVerified = doc.isVerified && !doc.isDemoPlaceholder;

              return (
                <div
                  key={doc.id}
                  className={`p-5 rounded-3xl bg-white dark:bg-slate-900 border shadow-xs hover:shadow-md transition-all flex flex-col justify-between relative overflow-hidden ${
                    isRealVerified
                      ? 'border-teal-500/50 dark:border-teal-700/50'
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className={`px-4 py-2.5 rounded-xl text-xs font-bold shadow-xs transition-all min-h-[44px] ${
                          isRealVerified
                            ? 'bg-teal-600 hover:bg-teal-700 text-white'
                            : 'bg-slate-700 hover:bg-slate-600 text-slate-200'
                        }`}
                      >
                        {isRealVerified ? 'Book Teleconsult' : 'Demo Consultation'}
                      </button>
                    </div>
                  </div>
                </div>
